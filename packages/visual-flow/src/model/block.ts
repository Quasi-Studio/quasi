...
export abstract class Block extends ModelBase {
  ...
  getDraggingSource(blockOnly: boolean): null | Block | [Socket, number] {
    ...
    if (hoveredSocket)
      return isInside ? [hoveredSocket[0], Number.NEGATIVE_INFINITY] : hoveredSocket
    else
      return isInside ? this : null
  }
...
function getNearestSocket(
  sockets: Socket[],
  blockPos: Point,
  maxSocketDistanceSquare: number = Number.POSITIVE_INFINITY,
): [Socket, number] | null {
