import type { ComponentInfo } from '@quasi-dev/runtime'
import { componentInfoObj } from '@quasi-dev/runtime'
import type {
  Socket,
  UseSocket,
...
export class ComponentBlock extends RectBlock {
  ...
  socketUpdater(useSocket: UseSocket): void {
    ...
    for (const [k, v] of contents) {
      if (v.mode === 'as-primary' || shouldHideSocket(k, v))
        continue
